 ...
 ***********************************************************************/
 // @TODO: set request. Action can be passed as MarketplaceWebServiceProducts_Model_GetMyPriceForSKURequest
...
/**
  * Get My Price For SKU Action Sample
  * Get my price
  *   
      ...
      try {
              $response = $service->getMyPriceForSKU($request);
                ...
                foreach ($getMyPriceForSKUResultList as $getMyPriceForSKUResult) {
                    echo("            GetMyPriceForSKUResult\n");
                if ($getMyPriceForSKUResult->isSetSellerSKU()) {
                    echo("        SellerSKU");
                    echo("\n");
                    echo("                " . $getMyPriceForSKUResult->getSellerSKU() . "\n");
                } 
                if ($getMyPriceForSKUResult->isSetStatus()) {
                    echo("        status");
                    echo("\n");
                    echo("                " . $getMyPriceForSKUResult->getStatus() . "\n");
                } 
                    ...
                    if ($getMyPriceForSKUResult->isSetError()) { 
                        echo("                Error\n");
                        $error = $getMyPriceForSKUResult->getError();
                        if ($error->isSetType()) 
                        {
                            echo("                    Type\n");
                            echo("                        " . $error->getType() . "\n");
